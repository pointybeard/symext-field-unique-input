<?php
	...
	Class fieldUniqueInput extends Field{
		...
		function __isHandleUnique($handle, $self_entry_id=NULL){
			return !(bool)$this->_engine->Database->fetchVar('id', 0, "SELECT `id` FROM `tbl_entries_data_" . $this->get('id') . "` WHERE `handle` = '$handle' ".(is_numeric($self_entry_id) ? " AND `entry_id` != $self_entry_id " : NULL) . "LIMIT 1");
		}
